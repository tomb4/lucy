...
func (this *FastLZCompressor) Compress(buffer []byte, bufferSize int32, compressed []byte, maxCompressedSize int32, compressedSize *int32) detour.DtStatus {
	*compressedSize = int32(Fastlz_compress(buffer, int(bufferSize), compressed))
...
func (this *FastLZCompressor) Decompress(compressed []byte, compressedSize int32, buffer []byte, maxBufferSize int32, bufferSize *int32) detour.DtStatus {
	*bufferSize = int32(Fastlz_decompress(compressed, int(compressedSize), buffer, int(maxBufferSize)))
	if *bufferSize < 0 {
...
func LoadDynamicMesh(path string) (*detour.DtNavMesh, *dtcache.DtTileCache, error) {
	meshData, err := ioutil.ReadFile(path)
	if err != nil {
		return nil, nil, err
	}
...
//func Distance(start, end Point) float32 {
//	distanceSqr := detour.DtVdistSqr(start.ToF(), end.ToF())
//	distance := math.Sqrt(float64(distanceSqr))
